...
function predictImage() {
    ...
    cv.threshold(image, image, 175, 255, cv.THRESH_BINARY); // increase contrast if color is less then 175(grey), make it white(255)
    // 3. get image contours
    let contours = new cv.MatVector(); // objects
    ...
    cv.copyMakeBorder(image, image, TOP, BOTTOM, LEFT, RIGHT, cv.BORDER_CONSTANT, BLACK); // add padding black color
    ...
    cv.warpAffine(image, image, M, newSize, cv.INTER_LINEAR, cv.BORDER_CONSTANT, BLACK); // shifting
    ...
    pixelValues = pixelValues.map(function (item) { // using map to apply fucntion to all elements in js array
        return item / 255.0; // dividing by 255
    });
